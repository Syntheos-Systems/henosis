...
| Indeterminate executions fail closed and cannot be retried automatically; there is no operator resolution command for them. | Open |
...
Machine and operator authentication, live membership checks, durable request-bound approvals, synchronous hash-chained audit, independent witness receipts, and the bounded Wasmtime host are implemented.
