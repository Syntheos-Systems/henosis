...
test_verified_install() {
    version=v0.1.0-alpha.5; target=x86_64-unknown-linux-musl; release="$TEST_ROOT/release"; case_root="$TEST_ROOT/success"
...
test_rollback() {
    version=v0.1.0-alpha.5; target=x86_64-unknown-linux-musl; release="$TEST_ROOT/rollback-release"; case_root="$TEST_ROOT/rollback"
...
test_checksum_failure() {
    version=v0.1.0-alpha.5; target=x86_64-unknown-linux-musl; release="$TEST_ROOT/checksum-release"; case_root="$TEST_ROOT/checksum"
    make_release "$version" "$target" 'exit 0' "$release"
    make_curl "$case_root/tools"; mkdir -p "$case_root/remote/$version"
    cp "$release/$version"/* "$case_root/remote/$version/"
...
test_mutable_release_failure() {
    version=v0.1.0-alpha.5; target=x86_64-unknown-linux-musl; release="$TEST_ROOT/mutable-release"; case_root="$TEST_ROOT/mutable"
    make_release "$version" "$target" 'exit 0' "$release"
    make_curl "$case_root/tools"; mkdir -p "$case_root/remote/$version"
    cp "$release/$version"/* "$case_root/remote/$version/"
...
test_release_metadata_structure() {
    version=v0.1.0-alpha.5; target=x86_64-unknown-linux-musl; release="$TEST_ROOT/metadata-release"
    make_release "$version" "$target" 'exit 0' "$release"
...
test_archive_local_install() {
    version=v0.1.0-alpha.5; target=x86_64-unknown-linux-musl; case_root="$TEST_ROOT/archive-local"
...
test_archive_marker_mismatch() {
    ...
    if "$case_root/archive/install.sh" --version v0.1.0-alpha.5 --install-dir "$case_root/bin" --headless > "$case_root/result.json" 2>&1; then
