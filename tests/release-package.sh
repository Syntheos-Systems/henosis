...
SOURCE_DATE_EPOCH=1784768092 "$REPOSITORY_DIR/scripts/package-release.sh" "$binary" "$crucible" 0.1.0-beta.1 x86_64-unknown-linux-musl "$TEST_DIRECTORY/dist" >/dev/null
archive="$TEST_DIRECTORY/dist/henosis-0.1.0-beta.1-x86_64-unknown-linux-musl.tar.gz"
[ -f "$archive" ] || fail 'archive was not created'
members=$(tar -tzf "$archive")
expected='henosis-0.1.0-beta.1-x86_64-unknown-linux-musl/
henosis-0.1.0-beta.1-x86_64-unknown-linux-musl/HENOSIS_ARCHIVE
henosis-0.1.0-beta.1-x86_64-unknown-linux-musl/LICENSE
henosis-0.1.0-beta.1-x86_64-unknown-linux-musl/README.md
henosis-0.1.0-beta.1-x86_64-unknown-linux-musl/crucible
henosis-0.1.0-beta.1-x86_64-unknown-linux-musl/henosis
henosis-0.1.0-beta.1-x86_64-unknown-linux-musl/install.sh'
[ "$members" = "$expected" ] || fail 'archive members differ from contract'
tar -xOf "$archive" henosis-0.1.0-beta.1-x86_64-unknown-linux-musl/README.md | grep -F 'henosis init --quick' >/dev/null || fail 'archive does not document initialization'
tar -xOf "$archive" henosis-0.1.0-beta.1-x86_64-unknown-linux-musl/README.md | grep -F 'offline installation' >/dev/null || fail 'archive does not document offline installation'
tar -xOf "$archive" henosis-0.1.0-beta.1-x86_64-unknown-linux-musl/README.md | grep -F 'crucible' >/dev/null || fail 'archive does not document Crucible'
[ "$(tar -xOf "$archive" henosis-0.1.0-beta.1-x86_64-unknown-linux-musl/HENOSIS_ARCHIVE)" = 'v0.1.0-beta.1 x86_64-unknown-linux-musl' ] || fail 'archive marker is incorrect'
