...
pub struct VerifyInput {
    /// Single command to run (legacy single-step form).
    pub command: Option<String>,
    /// Exit code that counts as success (defaults to 0).
    pub expected_exit_code: Option<i32>,
    /// Skill whose execution stats this run feeds (best-effort).
    pub skill_id: Option<i64>,
    /// Spec the verification belongs to.
    pub spec_id: Option<String>,
    /// Acceptance-criteria index this run verifies.
    pub criteria_index: Option<i64>,
    /// Per-command timeout in seconds.
    pub timeout_secs: Option<u64>,
    /// Multi-step form: commands run in order.
...
pub struct VerifyStep {
    ...
    /// Exit code that counts as success (defaults to 0).
    pub expected_exit_code: Option<i32>,
    /// Human label for the step in the report.
...
pub fn verify(
    db: &Database,
    bridge: Option<&dyn crate::bridge::SkillsBridge>,
    input: VerifyInput,
) -> ToolResult {
    ...
    // Skill recording (best-effort; no bridge = no recording).
...
pub struct ChallengeCodeInput {
    ...
    /// Aspects to focus the challenge on.
...
pub struct SessionDiffInput {
    /// Git base to diff against (defaults to HEAD).
