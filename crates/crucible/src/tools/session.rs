...
pub struct CheckpointInput {
    /// Checkpoint name (unique per session).
    pub name: Option<String>,
    /// What state the checkpoint captures.
...
pub struct RollbackInput {
    /// Name of the checkpoint to roll back to.
...
pub struct SessionLearnInput {
    ...
    /// Free-form tags for later recall.
    pub tags: Option<Vec<String>>,
    /// Also forward the discovery to skill capture (best-effort).
    pub capture_as_skill: Option<bool>,
    /// Spec this learning belongs to.
...
pub fn session_learn(
    db: &Database,
    bridge: Option<&dyn crate::bridge::SkillsBridge>,
    input: SessionLearnInput,
) -> ToolResult {
...
pub struct SessionRecallInput {
    /// Substring to match against recorded discoveries.
    pub query: Option<String>,
    /// Maximum learnings to recall.
