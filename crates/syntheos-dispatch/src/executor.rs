...
///
/// Object-safe via `async_trait` so the dispatcher can hold `Box<dyn Executor>`. The
/// fail-closed [`crate::deny::DenyExecutor`] is available when no executor is configured;
/// tests use the feature-gated `stubs::EchoExecutor`.
#[async_trait]
...
///
/// Carries a human-readable message plus a stable internal classification. Executors map their
/// own error types into this boundary type.
