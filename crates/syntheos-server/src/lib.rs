...
//! Chiasm, Soma, and Broca are wired: persistent stores open at boot, `/chiasm/tasks`
//! (+ `/chiasm/stats`) make tasks queryable, `/soma/agents` (register/list/get/heartbeat/quality,
//! + `/soma/stats`) makes agent presence queryable, and `/broca/actions` (log/feed/get/narrate,
//! + `/broca/stats`) makes the narration log queryable. The directory is the persistent
//! `SqliteDirectory`, so enrolled principals survive restarts alongside the projections that
//! reference them. Identity on these surfaces is caller-asserted (`principal_id` in the
//! body/query) until PistisGate lands in Phase 3 -- same posture as `/dispatch`'s
//! `RequestContext`.
//!
...
pub use app::{
    ...
    ChiasmListQuery, ChiasmOwnerQuery, EnrollRequest, SomaHeartbeatRequest, SomaListQuery,
    SomaQualityRequest, SomaRegisterRequest, SomaStatsQuery,
};
